...
@section('content')
                    ...
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Cabang</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Data Cabang</h1>
            </div>
            <div class="flex">
        ...
        <div
            ...
            <div class="flex items-center mb-4 sm:mb-0">
                <span class="text-xs font-normal text-gray-900 dark:text-white">Menampilkan <span
                    class="font-semibold text-gray-900 dark:text-white"> {{ $branch->firstItem() }}-{{ $branch->lastItem() }}</span> dari <span
                    class="font-semibold text-gray-900 dark:text-white">{{ $branch->total() }}</span></span>
            </div>
            <div class="flex items-center space-x-3">
                <a @if (1 !== $branch->currentPage()) href="{{ $branch->previousPageUrl() }}" @endif
                    class="@if (1 === $branch->currentPage()) cursor-not-allowed @endif inline-flex items-center justify-center flex-1 px-3 py-2 text-xs font-medium text-center text-customprimary-700 border border-customprimary-700 rounded-lg hover:text-white hover:bg-customprimary-700 focus:ring-4 focus:ring-customprimary-300 dark:bg-customprimary-600 dark:hover:bg-customprimary-700 dark:focus:ring-customprimary-800 dark:text-white">
                ...
                <a @if ($branch->lastPage() !== $branch->currentPage()) href="{{ $branch->nextPageUrl() }}" @endif
                    class="@if ($branch->lastPage() === $branch->currentPage()) cursor-not-allowed @endif inline-flex items-center justify-center flex-1 px-3 py-2 text-xs font-medium text-center text-customprimary-700 border border-customprimary-700 rounded-lg hover:text-white hover:bg-customprimary-700 focus:ring-4 focus:ring-customprimary-300 dark:bg-customprimary-600 dark:hover:bg-customprimary-700 dark:focus:ring-customprimary-800 dark:text-white">
                    Berikutnya
    ...
    @include('pages.dashboard.branch.add-form')
    @foreach ($branch as $data)
        @include('pages.dashboard.branch.detail', ['data' => $data])
        @include('pages.dashboard.general.delete', ['data' => $data, 'url' => route('dashboard.branch.destroy', ['id' => $data->id])])
    @endforeach
@endsection
