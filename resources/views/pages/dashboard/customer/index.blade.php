...
@section('content')
    <div
        ...
        <div class="w-full mb-1">
            <div class="mb-4">
                ...
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Data Customer</h1>
            </div>
            <div class="flex">
        ...
        <div
            ...
            <div class="flex items-center mb-4 sm:mb-0">
                <span class="text-xs font-normal text-gray-900 dark:text-white">Menampilkan <span
                    class="font-semibold text-gray-900 dark:text-white"> {{ $customer->firstItem() }}-{{ $customer->lastItem() }}</span> dari <span
                    class="font-semibold text-gray-900 dark:text-white">{{ $customer->total() }}</span></span>
            </div>
            <div class="flex items-center space-x-3">
                <a @if (1 !== $customer->currentPage()) href="{{ $customer->previousPageUrl() }}" @endif
                    class="@if (1 === $customer->currentPage()) cursor-not-allowed @endif inline-flex items-center justify-center flex-1 px-3 py-2 text-xs font-medium text-center text-customprimary-700 border border-customprimary-700 rounded-lg hover:text-white hover:bg-customprimary-700 focus:ring-4 focus:ring-customprimary-300 dark:bg-customprimary-600 dark:hover:bg-customprimary-700 dark:focus:ring-customprimary-800 dark:text-white">
                ...
                <a @if ($customer->lastPage() !== $customer->currentPage()) href="{{ $customer->nextPageUrl() }}" @endif
                    class="@if ($customer->lastPage() === $customer->currentPage()) cursor-not-allowed @endif inline-flex items-center justify-center flex-1 px-3 py-2 text-xs font-medium text-center text-customprimary-700 border border-customprimary-700 rounded-lg hover:text-white hover:bg-customprimary-700 focus:ring-4 focus:ring-customprimary-300 dark:bg-customprimary-600 dark:hover:bg-customprimary-700 dark:focus:ring-customprimary-800 dark:text-white">
                    Berikutnya
    ...
    @include('pages.dashboard.customer.add-form')

    @foreach ($customer as $data)
        @include('pages.dashboard.customer.detail', ['data' => $data])
        @include('pages.dashboard.general.delete', ['data' => $data, 'url' => route('dashboard.customer.destroy', ['id' => $data->id])])
    @endforeach
@endsection
