...
@section('content')
                    ...
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Display Slide</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Data Display Slide</h1>
            </div>
            <div class="flex">
                ...
                <div class="overflow-hidden shadow">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            @foreach ($diplayAboutSlides as $data)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td class="p-4 text-xs font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ Illuminate\Support\Str::words($data->description, 7, '...') }}
                                    </td>
        ...
        <div
            ...
            <div class="flex items-center mb-4 sm:mb-0">
                <span class="text-xs font-normal text-gray-900 dark:text-white">Menampilkan <span
                    class="font-semibold text-gray-900 dark:text-white"> {{ $diplayAboutSlides->firstItem() }}-{{ $diplayAboutSlides->lastItem() }}</span> dari <span
                    class="font-semibold text-gray-900 dark:text-white">{{ $diplayAboutSlides->total() }}</span></span>
            </div>
            <div class="flex items-center space-x-3">
                <a @if (1 !== $diplayAboutSlides->currentPage()) href="{{ $diplayAboutSlides->previousPageUrl() }}" @endif
                    class="@if (1 === $diplayAboutSlides->currentPage()) cursor-not-allowed @endif inline-flex items-center justify-center flex-1 px-3 py-2 text-xs font-medium text-center text-customprimary-700 border border-customprimary-700 rounded-lg hover:text-white hover:bg-customprimary-700 focus:ring-4 focus:ring-customprimary-300 dark:bg-customprimary-600 dark:hover:bg-customprimary-700 dark:focus:ring-customprimary-800 dark:text-white">
                ...
                <a @if ($diplayAboutSlides->lastPage() !== $diplayAboutSlides->currentPage()) href="{{ $diplayAboutSlides->nextPageUrl() }}" @endif
                    class="@if ($diplayAboutSlides->lastPage() === $diplayAboutSlides->currentPage()) cursor-not-allowed @endif inline-flex items-center justify-center flex-1 px-3 py-2 text-xs font-medium text-center text-customprimary-700 border border-customprimary-700 rounded-lg hover:text-white hover:bg-customprimary-700 focus:ring-4 focus:ring-customprimary-300 dark:bg-customprimary-600 dark:hover:bg-customprimary-700 dark:focus:ring-customprimary-800 dark:text-white">
                    Berikutnya
    ...
    {{-- @include('pages.dashboard.display-slide.add-form') --}}

    @foreach ($diplayAboutSlides as $data)
        @include('pages.dashboard.display-slide.detail', ['data' => $data])
        {{-- @include('pages.dashboard.general.delete', ['data' => $data, 'url' => route('dashboard.display-slide.destroy', ['id' => $data->id])]) --}}
    @endforeach
@endsection
