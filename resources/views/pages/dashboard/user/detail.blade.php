            ...
            <form action="{{ route('dashboard.user.update', ['id' => $data->id ]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 space-y-6 max-h-96 overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500">
                    <div class="grid grid-cols-1 gap-x-8 gap-y-4">
                        <div>
                            <label for="role_detail_{{ $data->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Jenis Pengguna</label>
                        ...
                        <div id="branchSelectDiv_detail_{{ $data->id }}" style="display: none;">
                            <label for="branch_id_{{ $data->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Cabang</label>
                            <select disabled name="branch_id" id="branch_id_{{ $data->id }}" class="block w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                required>
                                <option value="" disabled>-- Pilih Cabang --</option>
                                @foreach ($branch as $cabang)
                                    <option value="{{ $cabang->id }}" {{ (isset($data->branch_id) && $data->branch_id == $cabang->id) ? 'selected' : '' }}>{{ $cabang->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" disabled class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-customprimary-500 focus:border-customprimary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-customprimary-500 dark:focus:border-customprimary-500"> {{ $data->address }}</textarea>
                        </div>
                    </div>
                </div>
                <!-- Modal footer -->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.role-detail').forEach(roleSelect => {
            const idParts = roleSelect.id.split('_');
            const uniqueId = idParts[idParts.length - 1];
